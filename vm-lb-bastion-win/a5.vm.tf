...
resource "azurerm_linux_virtual_machine" "web_vm" {
  for_each = var.vm
  name     = "${local.resource_name_prefix}-vm-${each.key}" #add this line so that all the vm will have unique name 
  ...
  network_interface_ids = [
    azurerm_network_interface.web_nic[each.key].id, #attach nic to vm
  ]
